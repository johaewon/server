...
class VGDetection(torchvision.datasets.CocoDetection):
    def __init__(self, data_folder, feature_extractor, split, max_samples=None, debug=False):
        ...
        ## feasibility 확인을 위한 sample 개수 제한
    ...
    def __getitem__(self, idx):
        ...
        print(f"Loading item {idx}")  # 디버그 출력 추가
...
class VGDataset(VGDetection):
    def __init__(
        self, data_folder, feature_extractor, split, num_object_queries=100, max_samples=None, debug=False
    ):
        super(VGDataset, self).__init__(data_folder, feature_extractor, split, max_samples, debug)
        with open(f"{data_folder}/rel.json", "r") as f:
            rel = json.load(f)
        self.rel = rel[split]
        # no_relation 카테고리 제거 후 실제 관계만 고려
        self.rel_categories = rel["rel_categories"][1:]  # remove 'no_relation' category
        self.num_object_queries = num_object_queries
        
        ## feasibility 확인을 위한 sample 개수 제한
        if self.max_samples is not None: 
            self.rel = {k: self.rel[k] for k in list(self.rel.keys())[:self.max_samples]}
