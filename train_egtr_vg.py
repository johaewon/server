...
def check_duplicates(param_dicts, group_names):
    """디버깅을 위한 중복 파라미터 확인 함수."""
    seen_params = {}
    for i, (group, name) in enumerate(zip(param_dicts, group_names)):
        for n, p in group['params']:
            p_id = id(p)
            if p_id in seen_params:
                print(f"Duplicate parameter found in {name} (Group {i}): {p.shape}, Name: {n}")
                # 추가로 어떤 그룹에 이미 있었는지 확인
                print(f"Previously found in: {seen_params[p_id]}")
            else:
                seen_params[p_id] = name  # 파라미터의 참조를 그룹 이름과 함께 저장
...
if __name__ == "__main__":
    ...
    if not args.skip_train:
        ...
        if not Path(
            ...
            print("Before training memory usage:")
            ...
            print("After training memory usage:")
            print(torch.cuda.memory_summary())
